...
class Transaction(models.Model):
    ...
    status = models.CharField(max_length=20, default='Payment Done', choices=CHOICES, null=True, blank=False)
    created_at = models.DateTimeField(auto_now_add=True)
...
def invoice_amount_update(sender, instance, created, **kwargs):
    if created:
        ...
        if paid_amount == actual_inv_amount:
            status = 'payment_done'  # if the actual amount is same as paid amount payment is completed
        elif paid_amount <= remaining_amount:
